...
document.querySelectorAll(".tab").forEach(function(btn) {
  btn.addEventListener("click", function() {
    var isQb = btn.getAttribute("data-tab") === "qb";
    ...
    btn.classList.add(isQb ? "qb-act" : "act");
    document.getElementById("tab-" + btn.getAttribute("data-tab")).classList.add("act");
  });
});

function newRef(pfx) {
  var ts = Date.now().toString().slice(-8), rnd = Math.random().toString(36).slice(2,6).toUpperCase();
...
function setLoad(pfx, on) {
  var btn = document.getElementById(pfx + "PushBtn"), lbl = document.getElementById(pfx + "Lbl"), sp = document.getElementById(pfx + "Sp");
  btn.disabled = on; lbl.style.display = on ? "none" : "inline"; sp.style.display = on ? "block" : "none";
}

function fmtAmt(a, c) { var n = Number(a).toFixed(2); return c==="USD" ? "$"+n : c==="EUR" ? "\u20AC"+n : "AED "+n; }

document.getElementById("hsPushBtn").addEventListener("click", function() {
  var ref = document.getElementById("hsRef").value.trim(), amt = parseFloat(document.getElementById("hsAmt").value);
  var cur = document.getElementById("hsCur").value, st = document.getElementById("hsSt").value;
  if (!ref || isNaN(amt) || amt <= 0) { alert("Please fill in all fields."); return; }
  setLoad("hs", true); document.getElementById("hsRes").style.display = "none";
  fetch("/api/admin/test-hubspot", { method: "POST", headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ password: adminPwd, orderRef: ref, amount: amt, currency: cur, status: st }) })
    .then(function(r) { return r.json().then(function(d) { return { ok: r.ok, data: d }; }); })
    .then(function(r) { showRes("hs", r.ok && r.data.success, ref, amt, cur, st, r.data); setLoad("hs", false); })
    .catch(function(e) { showRes("hs", false, ref, amt, cur, st, { error: e.message }); setLoad("hs", false); });
});

document.getElementById("qbPushBtn").addEventListener("click", function() {
  var ref = document.getElementById("qbRef").value.trim(), amt = parseFloat(document.getElementById("qbAmt").value);
  var cur = document.getElementById("qbCur").value, st = document.getElementById("qbSt").value;
  if (!ref || isNaN(amt) || amt <= 0) { alert("Please fill in all fields."); return; }
  setLoad("qb", true); document.getElementById("qbRes").style.display = "none";
  fetch("/api/admin/test-quickbooks", { method: "POST", headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ password: adminPwd, orderRef: ref, amount: amt, currency: cur, status: st }) })
    .then(function(r) { return r.json().then(function(d) { return { ok: r.ok, data: d }; }); })
    .then(function(r) { showRes("qb", r.ok && r.data.success, ref, amt, cur, st, r.data); setLoad("qb", false); })
    .catch(function(e) { showRes("qb", false, ref, amt, cur, st, { error: e.message }); setLoad("qb", false); });
});

function showRes(pfx, ok, ref, amt, cur, st, data) {
  var box = document.getElementById(pfx + "Res"), ts = new Date().toLocaleString("en-AE"), html = "";
  if (ok) {
    var fields = pfx === "hs"
      ? [["Contact ID", (data.contact && data.contact.id) || "-", "bl"], ["Deal ID", (data.deal && data.deal.id) || "-", "gn"],
         ["Email", "customer-" + ref + "@nubexcloud.com", "or"], ["Amount", fmtAmt(amt, cur), "gn"], ["Order Ref", ref, ""], ["Status", st, ""]]
      : [["Customer ID", data.customerId || "-", "bl"], ["Invoice ID", data.invoiceId || "-", "gn"],
         ["Payment ID", data.paymentId || "-", "gn"], ["Amount", fmtAmt(amt, cur), "gn"], ["Order Ref", ref, ""], ["Status", st, ""]];
    var cells = fields.map(function(f) {
      return "<div class=\"rc2\"><div class=\"rl\">" + f[0] + "</div><div class=\"rval " + f[2] + "\">" + f[1] + "</div></div>";
    }).join("");
    var link = pfx === "hs"
      ? { cls: "hs", href: "https://app.hubspot.com/contacts/" + ((data.contact && data.contact.id) || ""), label: "Open in HubSpot \u2192" }
      : { cls: "qb", href: "https://app.qbo.intuit.com/app/invoice?txnId=" + (data.invoiceId || ""), label: "Open in QuickBooks \u2192" };
    html = "<div class=\"rc ok\">"
      + "<div class=\"rt ok\"><div class=\"ri2\">&#x2705;</div><div><div class=\"rv ok\">" + (pfx==="hs"?"HubSpot":"QuickBooks") + " \u2014 Success</div>"
      + "<div class=\"rs\">" + (pfx==="hs"?"Contact + Deal created":"Customer + Invoice + Payment created") + "</div></div></div>"
      + "<div class=\"rb2\">" + cells + "</div>"
      + "<div class=\"rf\"><a class=\"al " + link.cls + "\" href=\"" + link.href + "\" target=\"_blank\">" + link.label + "</a>"
      + "<div style=\"display:flex;align-items:center;gap:10px;\"><span class=\"ts\">" + ts + "</span><button class=\"ab\" id=\"ab_" + pfx + "\">\u2190 Test Again</button></div></div></div>";
  } else {
    html = "<div class=\"rc er\"><div class=\"rt er\"><div class=\"ri2\">&#x274C;</div><div><div class=\"rv er\">Push Failed</div>"
      + "<div class=\"rs\">" + ((data && data.error) || "Unknown error") + "</div></div></div>"
      + "<div class=\"rf\"><span class=\"ts\">" + ts + "</span><button class=\"ab\" id=\"ab_" + pfx + "\">\u2190 Try Again</button></div></div>";
  }
  box.innerHTML = html; box.style.display = "block";
  box.scrollIntoView({ behavior: "smooth", block: "nearest" });
  var ab = document.getElementById("ab_" + pfx);
  if (ab) ab.addEventListener("click", function() { newRef(pfx); box.style.display = "none"; });
}
...
function initSetup() {
  ...
  fetch("/health").then(function(r) { return r.json(); }).then(function(d) {
    var badge = document.getElementById("qbStatusBadge"), icon = document.getElementById("qbStatusIcon"), text = document.getElementById("qbStatusText");
    if (d.quickbooks === "connected") {
      badge.className = "ss cn"; icon.textContent = "\u2713";
      text.textContent = "Connected \u2014 tokens active (Realm: " + (d.qb_realm || "") + ")";
    } else {
      badge.className = "ss dc"; icon.textContent = "\u26A0";
