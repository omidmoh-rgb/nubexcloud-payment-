...
const CURRENCY       = (process.env.NGENIUS_CURRENCY      || "USD").trim();
...
// ── Payment Initiate (GET) ──────────────────────────────────────────────────
// Called by the middle/top-up page:
//   /payment-initiate?amount=50&currency=USD
// Converts dollars → cents, creates an N-Genius order, then redirects
// the customer straight to the N-Genius hosted payment page.
app.get("/payment-initiate", async (req, res) => {
  if (!API_KEY || !OUTLET_ID) {
    return res.status(500).send(`<!DOCTYPE html><html><head><meta charset="UTF-8"/>
      ...
      h2{color:#dc2626;margin-bottom:10px;}p{color:#374151;font-size:14px;}
      a{color:#f97316;font-weight:bold;}</style></head>
      <body><div class="box"><h2>⚠️ Server Not Configured</h2>
      <p>Payment credentials are missing. Please contact support.</p>
      <a href="/">← Go Back</a></div></body></html>`);
  }

  // amount arrives in dollars (e.g. "50" for $50)
  const amountDollars = parseFloat(req.query.amount);
  const currency      = ((req.query.currency || "USD").trim()).toUpperCase();
  ...
  if (!amountDollars || isNaN(amountDollars) || amountDollars <= 0) {
    return res.status(400).send(`<!DOCTYPE html><html><head><meta charset="UTF-8"/>
      ...
      h2{color:#dc2626;margin-bottom:10px;}p{color:#374151;font-size:14px;}
      a{color:#f97316;font-weight:bold;}</style></head>
      <body><div class="box"><h2>⚠️ Invalid Amount</h2>
      <p>Please go back and select a valid amount.</p>
      <a href="/">← Go Back</a></div></body></html>`);
  }

  // N-Genius requires amount in minor units (cents): $1.00 → 100
  const amountCents  = Math.round(amountDollars * 100);
  const redirectBase = getRedirectBase({});

  console.log(`\n[PAYMENT-INITIATE] ${currency} ${amountDollars} -> ${amountCents} cents | redirect: ${redirectBase}`);

  try {
    const token  = await getAccessToken();
    const result = await tryCreateOrder(token, amountCents, currency, redirectBase);
    console.log(`  [PAYMENT-INITIATE] Redirecting to N-Genius: ${result.paymentUrl}`);
    ...
    return res.status(500).send(`<!DOCTYPE html><html><head><meta charset="UTF-8"/>
      ...
      <style>body{font-family:Arial,sans-serif;background:#f8fafc;display:flex;align-items:center;justify-content:center;min-height:100vh;margin:0;}
      .box{background:#fff;padding:40px;border-radius:12px;box-shadow:0 4px 20px rgba(0,0,0,.08);text-align:center;max-width:480px;}
      h2{color:#dc2626;margin-bottom:10px;}p{color:#374151;font-size:14px;line-height:1.6;}
      .err{background:#fef2f2;border:1px solid #fecaca;border-radius:8px;padding:12px;font-size:12px;font-family:monospace;color:#dc2626;margin:16px 0;word-break:break-all;text-align:left;}
      a{color:#f97316;font-weight:bold;}</style></head>
      <body><div class="box"><h2>⚠️ Payment Failed</h2>
      <p>We could not process your payment request.</p>
