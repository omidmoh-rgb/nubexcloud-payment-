...
function showResult(type,ref,amount,currency,status,data){
  ...
  if(type==="ok"){
    box.innerHTML='<div class="rc ok">'
      ...
      +'</div>'
      +'<div class="rc-foot">'
      +'<a class="hs-link" id="hsOpenLink" href="https://app.hubspot.com/contacts/'+(data.contact&&data.contact.id||'')+'" target="_blank">Open in HubSpot &rarr;</a>'
      +'<div style="display:flex;align-items:center;gap:10px;"><span class="ts">'+ts+'</span>'
      +'<button class="again-btn" id="againBtn">&larr; Test Again</button>'
      +'</div></div></div>';
    document.getElementById("againBtn").addEventListener("click",function(){ regenRef(); hideResult(); });
  } else {
    box.innerHTML='<div class="rc err">'
      ...
      +'<button class="again-btn" id="againBtn">&larr; Try Again</button>'
      +'</div></div>';
    document.getElementById("againBtn").addEventListener("click", hideResult);
  }
